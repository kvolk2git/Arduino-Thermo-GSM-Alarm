...
#include <EEPROM.h>
#include <OneWire.h>
const String sid = "GSM Temperature Alarm 2019.01.07-01";
typedef struct  {
  char PhoneNum[17];
  uint8_t SAlarm;
  uint8_t RAlarm;
...
void eepromupdatelowtemp(int8_t SmsLowTemp) {
  EEPROM.update(20, SmsLowTemp);
  LowTemperature = SmsLowTemp;
  Serial.print( F ("LowTemp is: "));
  Serial.println((int8_t)EEPROM.read(20));
  lineprint();
  LedOK();
}

// ----- Изменение верхней границы температуры
void eepromupdatehightemp(int8_t SmsHighTemp) {
  EEPROM.update(21, SmsHighTemp);
  HighTemperature = SmsHighTemp;
  Serial.print( F ("HighTemp is: "));
  Serial.println((int8_t)EEPROM.read(21));
  lineprint();
  LedOK();
}
...
void eepromconfig() {
  ...
  if (EEPROM.read(0) == 1) {
    ...
    LowTemperature = (int8_t)EEPROM.read(20); //Нижняя граница температуры до -50
    Serial.print( F ("LowTemp: "));
    Serial.println(LowTemperature);

    HighTemperature = (int8_t)EEPROM.read(21); //Верхняя граница температуры до 125
